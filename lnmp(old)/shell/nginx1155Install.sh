...
function BaseInstall() {
    RESOURCE_PATH=$1
    #ls $RESOURCE_PATH/nginx-1.15.5.tar.gz &> /dev/null
    # if [ $? -ne 0 ];then
    #     wget http://nginx.org/download/nginx-1.15.5.tar.gz -O /data/download/nginx-1.15.5.tar.gz
    # fi
    cd $RESOURCE_PATH && tar -xvf nginx-1.15.5.tar.gz -C /usr/local/src
    cd /usr/local/src/nginx-1.15.5
    #stub_status/性能統計模塊

    ./configure \
    --prefix=/usr/local/nginx1155 \
    --sbin-path=/usr/local/nginx1155/sbin/nginx \
    --conf-path=/usr/local/nginx1155/conf/nginx.conf \
    --error-log-path=/usr/local/nginx1155/log/error.log \
    --http-log-path=/usr/local/nginx1155/log/access.log \
    --pid-path=/usr/local/nginx1155/runtime/nginx.pid \
    --lock-path=/usr/local/nginx1155/runtime/nginx.lock \
    --user=nginx \
    --group=nginx \
    --with-mail \
    --with-stream \
    ...
    --http-scgi-temp-path=/usr/local/nginx1155/runtime/scgi \
    --http-uwsgi-temp-path=/usr/local/nginx1155/runtime/uwsgi \
    --http-proxy-temp-path=/usr/local/nginx1155/runtime/proxy \
    --http-fastcgi-temp-path=/usr/local/nginx1155/runtime/fastcgi \
    --http-client-body-temp-path=/usr/local/nginx1155/runtime/client_body \
...
function Configuration {
    ...
    mkdir -p /usr/local/nginx1155/conf/virtualhost
...
http {
    ...
    include             /usr/local/nginx1155/conf/virtualhost/*.conf;
    #配置虛擬主機，start-----
    #配置虛擬主機，end-----
}' > /usr/local/nginx1155/conf/nginx.conf
...
#虛擬主機1，end-----' > /usr/local/nginx1155/conf/virtualhost/example.txt
...
[Service] 
Type=forking 
ExecStart=/usr/local/nginx1155/sbin/nginx
ExecReload=/usr/local/nginx1155/sbin/nginx reload
ExecStop=/usr/local/nginx1155/sbin/nginx quit
PrivateTmp=true 
[Install] 
WantedBy=multi-user.target' > /lib/systemd/system/nginx1155.service
    chmod 754 /usr/lib/systemd/system/nginx1155.service
    systemctl daemon-reload
    systemctl enable nginx1155.service
    systemctl restart nginx1155.service
    ln -sf /usr/local/nginx1155/sbin/nginx /usr/local/bin
}
