...
function PrepareEnvironment() {
	...
	yum -y --nogpgcheck install libxml2 libxml2-devel openssl openssl-devel curl-devel libjpeg-devel libpng-devel freetype-devel libmcrypt libmcrypt-devel bzip2 bzip2-devel re2c libiconv mhash openssl-dev
...
;設置擴展
extension_dir = "/usr/local/'${BIN_NAME}'/lib/extensions/no-debug-non-zts-20170718"
extension = amqp.so
extension = geoip.so
extension = redis.so
extension = swoole.so
...
slowlog = /usr/local/'${BIN_NAME}'/log/php-fpm-slow.log' > /usr/local/${BIN_NAME}/etc/php-fpm.conf
	...
	cp /usr/local/src/${SRC_NAME}/sapi/fpm/php-fpm.service /lib/systemd/system/php-fpm-7280.service
	chmod 745 /lib/systemd/system/php-fpm-7280.service
	systemctl enable php-fpm-7280.service
	systemctl restart php-fpm-7280.service
...
function ExtensionInstall() {
	...
	# fileinfo,start-----
	cd /usr/local/src/${SRC_NAME}/ext/fileinfo
	...
	# swoole,start-----
	# wget http://pecl.php.net/get/swoole-4.3.5.tgz
	cd $RESOURCE_PATH && tar -xvf swoole-4.4.7.tgz -C /usr/local/src/PHPExtension/${BIN_NAME} && cd /usr/local/src/PHPExtension/${BIN_NAME}/swoole-4.4.7
	/usr/local/${BIN_NAME}/bin/phpize
	./configure \
	--with-php-config=/usr/local/${BIN_NAME}/bin/php-config \
	--enable-http2  \
	...
	ls /usr/local/rabbitmq-c
	if [ $? -ne 0 ];then
		cd $RESOURCE_PATH && tar -xvf rabbitmq-c-0.9.0.tar.gz -C /usr/local/src/PHPExtension/${BIN_NAME} && cd /usr/local/src/PHPExtension/${BIN_NAME}/rabbitmq-c-0.9.0
	...
	cd $RESOURCE_PATH && tar -xvf amqp-1.9.4.tgz -C /usr/local/src/PHPExtension/${BIN_NAME} && cd /usr/local/src/PHPExtension/${BIN_NAME}/amqp-1.9.4
	/usr/local/${BIN_NAME}/bin/phpize
	./configure \
	--with-php-config=/usr/local/${BIN_NAME}/bin/php-config \
	--with-amqp \
	...
	#yaconf,start-----
	cd $RESOURCE_PATH && tar -xvf yaconf-1.0.7.tgz -C /usr/local/src/PHPExtension/${BIN_NAME} && cd /usr/local/src/PHPExtension/${BIN_NAME}/yaconf-1.0.7
	/usr/local/${BIN_NAME}/bin/phpize
	./configure \
	--with-php-config=/usr/local/${BIN_NAME}/bin/php-config
	make -j8 && make install
	...
	cd $RESOURCE_PATH && tar -zxf xhprof-2.1.3.tgz -C /usr/local/src/PHPExtension/${BIN_NAME} && cd /usr/local/src/PHPExtension/${BIN_NAME}/xhprof-2.1.3
	#TODO : cp -r xhprof_html xhprof_lib /{websiteRoot}
	cd extension
	/usr/local/${BIN_NAME}/bin/phpize
	./configure \
	--with-php-config=/usr/local/${BIN_NAME}/bin/php-config
