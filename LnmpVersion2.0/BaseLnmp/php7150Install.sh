...
slowlog = /usr/local/'${BIN_NAME}'/log/php-fpm-slow.log' > /usr/local/${BIN_NAME}/etc/php-fpm.conf
	...
	cp /usr/local/src/${SRC_NAME}/sapi/fpm/php-fpm.service /lib/systemd/system/php-fpm-7150.service
	chmod 745 /lib/systemd/system/php-fpm-7150.service
	systemctl enable php-fpm-7150.service
	systemctl restart php-fpm-7150.service
...
function ExtensionInstall() {
	...
	cd $RESOURCE_PATH && tar -xvf swoole-4.3.5.tgz -C /usr/local/src/PHPExtension/${BIN_NAME} && cd /usr/local/src/PHPExtension/${BIN_NAME}/swoole-4.3.5
	/usr/local/${BIN_NAME}/bin/phpize
	./configure \
	--with-php-config=/usr/local/${BIN_NAME}/bin/php-config \
	--enable-http2  \
